 ...
 * @author Bhaumik
 */
public class CountryMaster extends javax.swing.JInternalFrame {
    ...
    public CountryMaster(int user_id) {
        initComponents();
        form_cd = user_id;
        connectToNavigation();
    ...
    public void setID(String code) {
        country_cd = Integer.parseInt(code);
        navLoad.setVoucher("Edit");
    }

    private void makeViewTable() {
        table = new ReportTable();
        table.AddColumn(0, "Country Code", 0, java.lang.Integer.class, null, false);
        table.AddColumn(1, "Country Name", -1, java.lang.String.class, null, false);
        table.makeTable();
    }
    ...
    private void connectToNavigation() {
        class navPanel extends SmallNavigation {
                    ...
                    protected Object doInBackground() throws Exception {
                        try {
                            ...
                            if (getMode().equalsIgnoreCase("N")) {
                                sql = " insert into countrymst (country_name,user_id)values (?,?)";
                            } else if (getMode().equalsIgnoreCase("E")) {
                                sql = "update countrymst set country_name=?,user_id=?,edit_no=edit_no+1,time_stamp=current_timestamp"
            ...
            public void callDelete() {
                SwingWorker workerForjbtnGenerate = new SwingWorker() {
                    ...
                    protected Object doInBackground() throws Exception {
                        try {
                            lb.addGlassPane(navLoad);
                            if (lb.isExist("statemst", "country_cd", country_cd + "", dataConnection)) {
                                navLoad.setMessage("Country exist in state master. you can not delete this country");
                            } else {
                                lb.confirmDialog("Do you want to delete this country?");
                                if (lb.type) {
                                    String sql = "delete from countrymst where country_cd=" + country_cd;
                                    PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
                                    pstLocal.executeUpdate();
                                    setVoucher("Previous");
                                }
            ...
            public void callView() {
                String sql = "select country_cd,country_name from countrymst";
                makeViewTable();
                HeaderIntFrame header = new HeaderIntFrame(dataConnection, country_cd + "", "Country Master View", sql, "10", 1, CountryMaster.this, "Country Master", table);
                header.makeView();
                cancelOrClose();
                HMSHome.addOnScreen(header, "Country Master View",-1);
            }
            ...
            public void setVoucher(String tag) {
                ...
                if (tag.equalsIgnoreCase("First")) {
                    viewData = fetchData("select * from countrymst where country_cd=(select min(country_cd) from countrymst)");
                } else if (tag.equalsIgnoreCase("Previous")) {
                    viewData = fetchData("select * from countrymst where country_cd=(select max(country_cd) from countrymst where country_cd <" + country_cd + ")");
                } else if (tag.equalsIgnoreCase("Next")) {
                    viewData = fetchData("select * from countrymst where country_cd=(select min(country_cd) from countrymst where country_cd >" + country_cd + ")");
                } else if (tag.equalsIgnoreCase("Last")) {
                    viewData = fetchData("select * from countrymst where country_cd=(select max(country_cd) from countrymst)");
                } else if (tag.equalsIgnoreCase("edit")) {
                    viewData = fetchData("select * from countrymst where country_cd=" + country_cd);
                }
                try {
                    if (viewData.next()) {
            ...
            public void setComponentTextFromResultSet() {
                try {
                    country_cd = viewData.getInt("country_cd");
                    jtxtCountryName.setText(viewData.getString("country_name"));
            ...
            public boolean validateVoucher() {
                if (lb.isBlank(jtxtCountryName)) {
                    setMessage("Country name can not be left blank");
                    jtxtCountryName.requestFocusInWindow();
                    return false;
                }

                if (getMode().equalsIgnoreCase("N")) {
                    if (lb.isExist("countrymst", "country_name", jtxtCountryName.getText(), dataConnection)) {
                        setMessage("Country name already exist");
                        jtxtCountryName.requestFocusInWindow();
                        return false;
                    }
                } else if (getMode().equalsIgnoreCase("E")) {
                    if (lb.isExistForEdit("countrymst", "country_name", jtxtCountryName.getText(), "country_cd", country_cd + "", dataConnection)) {
                        setMessage("Country name already exist");
                        jtxtCountryName.requestFocusInWindow();
                        return false;
                    }
