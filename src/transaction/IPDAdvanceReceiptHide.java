 ...
 * @author Lenovo
 */
public class IPDAdvanceReceiptHide extends javax.swing.JInternalFrame {
    ...
    IPDBillGenerationDischargeHide ipd = null;
    String user_id="";

    /**
     * Creates new form IPDAdvanceReceipt
     */
    public IPDAdvanceReceiptHide(int i) {
        initComponents();
        mode = i;
        addNavigationPanel();
    ...
    public IPDAdvanceReceiptHide(String ipdNUmber, int i,IPDBillGenerationDischargeHide ipd,String user_id) {
        initComponents();
    ...
    private void addNavigationPanel() {
        class navPanel extends NavigationPanel {
            ...
            public void callSave() throws Exception {
                ...
                if (pd.getReturnStatus() == pd.RET_OK) {
                    ...
                    if (getMode().equalsIgnoreCase("N")) {
                        ...
                        ref_no = lb.generateKey("ipdpaymenthd", "ref_no", 9, "ARHD" + HMSHome.year + "/");
            ...
            public void callView() {
                ...
                sql += " order by v_date";
                makeViewTable();
                HeaderIntFrame header = new HeaderIntFrame(dataConnection, ref_no + "", "Advanced Receipt View", sql, "24", 1, IPDAdvanceReceiptHide.this, "Advanced Receipt", table);
                header.makeView();
                cancelOrClose();
            ...
            public void setVoucher(String tag) {
                ...
                if (mode == 1) {
                    if (tag.equalsIgnoreCase("First")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join payment p on a.ref_no=p.ref_no where a.amount >0 and  a.ref_no=(select min(ref_no) from " + tableHD + " where amount >0 and ref_no like 'ARHD%')");
                    } else if (tag.equalsIgnoreCase("Previous")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join  payment p on a.ref_no=p.ref_no where a.amount >0 and  a.ref_no=(select max(ref_no) from " + tableHD + " where amount >0 and ref_no <'" + ref_no + "' and ref_no like 'ARHD%') and ref_no like 'ARHD%'");
                    } else if (tag.equalsIgnoreCase("Next")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join payment p on a.ref_no=p.ref_no where a.amount >0 and  a.ref_no=(select min(ref_no) from " + tableHD + " where amount >0 and ref_no >'" + ref_no + "' and ref_no like 'ARHD%') and ref_no like 'ARHD%'");
                    } else if (tag.equalsIgnoreCase("Last")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join payment p on a.ref_no=p.ref_no where a.amount >0 and a.ref_no=(select max(ref_no) from " + tableHD + " where amount >0 and ref_no like 'ARHD%')");
                    } else if (tag.equalsIgnoreCase("edit")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join payment p  on a.ref_no=p.ref_no where a.amount >0 and  a.ref_no='" + ref_no + "' and ref_no like 'ARHD%'");
                    }
                } else if (mode == -1) {
                    if (tag.equalsIgnoreCase("First")) {
                        viewDataRs = fetchData("select * from " + tableHD + " a  left join payment p on a.ref_no=p.ref_no where a.amount <0 and  a.ref_no=(select min(ref_no) from " + tableHD + " where amount <0 and ref_no like 'ARHD%')");
    ...
    private void jbtnOldVoucherActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnOldVoucherActionPerformed
        // TODO add your handling code here:
        if (mode > 0) {
            IPDAdvanceReceiptHide opd = new IPDAdvanceReceiptHide(1);
