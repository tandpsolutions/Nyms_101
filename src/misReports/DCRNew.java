 ...
 * @author Lenovo
 */
public class DCRNew extends javax.swing.JInternalFrame {
    ...
    private void addUserData() {
        try {
            ...
            String sql = "SELECT DISTINCT(l.user_name) FROM login l LEFT JOIN ipdpaymenthd i ON i.user_id=l.user_id where i.v_date >='" + lb.ConvertDateFormetForDB(jtxtFromDate.getText()) + "' and "
                    + "  i.v_date <='" + lb.ConvertDateFormetForDB(jtxtToDate.getText()) + "'";
            PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
            ResultSet rsLocal = pstLocal.executeQuery();
            while (rsLocal.next()) {
                data.add(rsLocal.getString("user_name"));
                model.addElement(new JCheckBox(rsLocal.getString("user_name")));
            }
            sql = "SELECT DISTINCT(l.user_name) FROM login l LEFT JOIN opdpaymenthd i ON i.user_id=l.user_id where i.v_date >='" + lb.ConvertDateFormetForDB(jtxtFromDate.getText()) + "' and "
                    + "  i.v_date <='" + lb.ConvertDateFormetForDB(jtxtToDate.getText()) + "'";
            pstLocal = dataConnection.prepareStatement(sql);
            rsLocal = pstLocal.executeQuery();
            while (rsLocal.next()) {
                if (!data.contains(rsLocal.getString("user_name"))) {
                    data.add(rsLocal.getString("user_name"));
                    model.addElement(new JCheckBox(rsLocal.getString("user_name")));
                }
            }
            sql = "SELECT DISTINCT(l.user_name) FROM login l LEFT JOIN opdbillhd i ON i.user_id=l.user_id where i.v_date >='" + lb.ConvertDateFormetForDB(jtxtFromDate.getText()) + "' and "
                    + "  i.v_date <='" + lb.ConvertDateFormetForDB(jtxtToDate.getText()) + "'";
            pstLocal = dataConnection.prepareStatement(sql);
            rsLocal = pstLocal.executeQuery();
            while (rsLocal.next()) {
